...
export default function LoginPage({ onLoginSuccess }) {
  ...
  const handleEmailAuth = async (e) => {
    e.preventDefault();
    try {
      const endpoint = isSignup ? "http://localhost:5050/auth/signup" : "http://localhost:5050/auth/login";
    ...
    <div style={styles.container}>
      ...
      <div style={styles.card}>
        ...
        <p style={styles.toggle}>
          ...
          <span onClick={() => setIsSignup(!isSignup)} style={styles.link}>{isSignup ? "Login" : "Sign Up"}</span>
        </p>
